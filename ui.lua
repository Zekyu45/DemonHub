...
function ui.createCustomUI(toggleAfk, showNotifications, antiAfkEnabled)
    ...
    -- Variable locale pour stocker l'état actuel des options
    local uiState = {
    ...
    antiAfkButton.MouseButton1Click:Connect(function()
        uiState.antiAfkEnabled = not uiState.antiAfkEnabled
        toggleAfk(uiState.antiAfkEnabled)
        
        if uiState.antiAfkEnabled then
    ...
    notifButton.MouseButton1Click:Connect(function()
        uiState.showNotifications = not uiState.showNotifications
...
function ui.startKeySystem(correctKey, onSuccess)
    ...
    submitButton.MouseButton1Click:Connect(function()
        ...
        if enteredKey == correctKey:lower() then
            ui.notify("Succès!", "Clé validée avec succès", 3)
            task.wait(1)
            keyGui:Destroy()
            task.wait(0.5)
            if onSuccess then
            ...
            ui.notify("Erreur!", "Clé d'activation incorrecte", 3)
            keyInput.Text = ""
        end
    end)
